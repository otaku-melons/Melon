# Настройка логов
Благодаря унификации порталов CLI/логов Melon может поставлять расширенные средства работы с такого рода данными. Для каждого парсера при первом запуске генерируется файл _Configs/{PARSER}/logger.json_, который и определяет поведение системы.

## telebot
Melon позволяет отправлять отчёты об ошибках в Telegram. Данная секция позволяет настроить отсылку и фильтрацию данных.

Помните, что Melon обрабатывает записи в логи только через поставляемые порталы, потому ошибки и предупреждения из сторонних библиотек не могут быть отправлены на данный момент.
___
```JSON
"enable": false
```
...
"chat_id": 0
```
Сюда необходимо занести ID чата Telegram, в который будут направляться отчёты (можно узнать у [Chat ID Bot](https://t.me/chat_id_echo_bot)). Если чат групповой, бот должен являться администратором. Для отправки в личные сообщения следует указать ID аккаунта.
___
```JSON
"comment": ""
```
