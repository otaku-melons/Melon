...
| Заголовок главы | Полная подпись главы, включающая нумерацию, типизацию и названия, предоставляемая источником (например, _Том 1. Эпилог: Название_). |

## 1. Инициализация рабочей директории
Melon поддерживает два типа контента: манга и ранобэ.

Воспользуйтесь командой `melon init -p {PARSER} --content {TYPES}` для автоматической генерации всех необходимых файлов парсера выбранного типа контента, где `PARSER` – название парсера, а `TYPE` – типы контента через запятую (_manga_, _ranobe_). 

> [!IMPORTANT]   
> Название парсера должно содержать только символы латинского алфавита в нижнем регистре и цифры!

Будет создан каталог `Parsers/{parser_name}`, в котором появятся следующие файлы:

### .gitignore
По умолчанию игнорируется кэш Python.

### main.py
Внутри находится класс **SourceOperator**, унаследованный от базового оператора, в котором реулизуются методы взаимодействия с сайтом-источником.

### manga.py | ranobe.py
Внутри находится класс **Parser**, унаследованный от соответствующего типа парсера, в котором определяются методы получения контента.
...
* `$from_parent` – берёт версию родительского парсера;
...
### settings.json
Определите кастомные параметры в секции `custom` и опишите их в [README.md](#readmemd), а также вы можете задать значения по умолчанию для стандартизированных настроек. При установке поддерживается вычисление дельт, потому следует оставить в файле только те секции и поля, которые отличаются от стандартных параметров.
...
Для удобства в сгенерированных __main.py__, __manga.py__ и __ranobe.py__ будут находиться все доступные для переопределения методы и импортированы все существующие вспомогательные модули, но те из них, которые не требуются для функционирования парсера, следует удалить.

### SourceOperator
В операторе источника определяются методы сборки алиасов тайтла, получения алиасов из строк и инициализации менеджера запросов.

#### Переопределяемые методы
| **Метод** | **Описание** |
|---|---|
| _InitializeRequestor | Инициализирует оператор запросов. Переопределите для использования собственных конфигураций. Изменения также будут подхвачены во всех зависимых модулях (например, при сборке контента или скачивании изображений). |
| _PostInitMethod | Вызывается в конце `__init__()` класса. Используйте его как замену, чтобы не переопределять конструктор. |

#### Публичные методы
| **Метод** | **Описание** |
|---|---|
| collect | Просит парсер собрать список алиасов по трём заданным опциям:<br><br>**period** – период сбора обновлений в часах;<br><br>**filters** – строка для дополнительных способов фильтрации (обычно параметры запроса), описывается индивидуально в [README.md](#readmemd);<br><br>**pages** – количество страниц каталога, с которых нужно получить данные. <br><br>Парсер может не поддерживать все опции сразу! |
| get_slug_from_string | Извлекает из переданной строки алиас тайтла. Автоматически вызывается для всех строк, переданных в слот алиаса команды парсинга. |
| image | Скачивает изображение во временный каталог парсера. |

### Parser
В классе парсера определяются методы, направленные на заполнение полей определённого формата данных.
...
| postprocessor | Выполняется непосредственно перед сохранением тайтла (например, для очистки временных данных, прикрепляемых к структуре главы на уровне словаря). |
...
## 4. Заимствуемые модули
Во время разработки парсеров часто приходится использовать похожие или вовсе одинаковые алгоритмы, но при этом требующие тонкой настройки и точечного расширения функционала. Melon предоставляет набор инструментов, который может стать полезным в некоторых ситуациях.

### Словари ключевых слов
При работе с некоторыми тайтлами может возникнуть необходимость обрабатывать ключевые слова, например при парсинге нумерации из строки типа «_Том 1. Глава 1. Название_» слова _Том_ и _Глава_ являются ключевыми и используются внутренними системами Melon для автоматического извлечения данных при обработке ранобэ. Вы можете переопределить их для корректной работы.
```Python
...
WordsDictionary.additional_data["custom"] = "Ключевое слово"
```

### Парсер заголовков глав
Источники контента не всегда предоставляют удобные способы получения данных наподобие номера тома и главы, в связи с чем периодически возникает необходимость парсить заголовки глав. Для этого вы можете использовать парсер заголовков, поддерживающий расширение через наследование.
```Python
from Source.Core.Base.Parsers.Components.ChapterHeaderParser import RanobeHeaderParser

# Парсинг заголовка с использованием словаря ключевых слов.
ParsedHeader = RanobeHeaderParser("Том 1. Эпилог: Название", self._Title.words_dictionary).parse()
# Доступ к данным.
ParsedHeader.volume
ParsedHeader.number
ParsedHeader.type
...
### Функции
Для упрощения парсинга в Melon встроена небольшая библиотека полезных функций.
```Python
...
Paragraphs = Functions.SplitParagraph(Soup, Soup.find("p"))
```

### Сортировщик глав
Алгоритм базовой сортировки глав основан на их нумерации, но не всегда это наилучшее решение. Если необходимо реализовать собственный сценарий, вы можете унаследовать объект от `Branch` и переопределить метод `sort()`. Для создания ветвей в дальнейшем используйте собственный класс.
